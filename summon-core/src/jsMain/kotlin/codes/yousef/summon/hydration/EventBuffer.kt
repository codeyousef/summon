...
/**
 * Buffer for capturing user events during hydration.
 *
...
class EventBuffer private constructor() {
    ...
    fun captureEvent(event: CapturedEvent) {
        ...
        while (queue.size >= maxEventsPerTarget) {
            ...
            if (enableLogging) {
                console.log("[EventBuffer] Dropped oldest event for ${event.targetId}: ${removed.type}")
            }
        }
        ...
        if (enableLogging) {
            console.log("[EventBuffer] Captured ${event.type} for ${event.targetId} (queued: ${queue.size})")
        }
    }
    ...
    fun replayEventsFor(targetId: String, handler: (CapturedEvent) -> Unit) {
        ...
        while (queue.isNotEmpty()) {
            ...
            if (event.isStale(maxEventAgeMs, currentTime)) {
                staleCount++
                if (enableLogging) {
                    console.log("[EventBuffer] Discarded stale ${event.type} for $targetId")
                }
                continue
            }
            ...
            try {
                ...
                console.error("[EventBuffer] Error replaying ${event.type} for $targetId: ${e.message}")
            }
        }

        if (enableLogging && (replayedCount > 0 || staleCount > 0)) {
            console.log("[EventBuffer] Replayed $replayedCount events for $targetId (discarded $staleCount stale)")
        }
    }
    ...
    fun pruneStaleEvents() {
        ...
        if (enableLogging && prunedCount > 0) {
            console.log("[EventBuffer] Pruned $prunedCount stale events")
        }
    }
 ...
 */
private external val console: EventBufferConsole

private external interface EventBufferConsole {
