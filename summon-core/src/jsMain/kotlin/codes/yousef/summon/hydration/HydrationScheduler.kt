...
/**
 * Non-blocking scheduler for hydration tasks.
 *
...
class HydrationScheduler private constructor() {
    ...
    fun scheduleTask(task: HydrationTask) {
        if (enableLogging) {
            console.log("[HydrationScheduler] Scheduling task: ${task.id} (priority: ${task.priority}, syncMode: $syncMode)")
        }
        ...
        if (enableLogging) {
            console.log("[HydrationScheduler] Scheduled task: ${task.id} (priority: ${task.priority})")
        }
    ...
    fun start() {
        ...
        if (enableLogging) {
            console.log("[HydrationScheduler] Starting with ${taskQueue.size} tasks (idleCallback: $useIdleCallback, scrollYielding: $enableScrollYielding)")
        }
    ...
    private fun attachScrollListener() {
        ...
        kotlinx.browser.window.addEventListener("scroll", { _ ->
            if (enableScrollYielding) {
                scrollPauseUntil = performanceNow() + scrollPauseDurationMs
                if (enableLogging) {
                    console.log("[HydrationScheduler] Scroll detected, pausing hydration for ${scrollPauseDurationMs.toInt()}ms")
                }
            }
        }, js("{ passive: true }").unsafeCast<dynamic>())
    }
    ...
    private fun scheduleNextBatch() {
        if (!isProcessing || taskQueue.isEmpty()) {
            isProcessing = false
            if (enableLogging) {
                console.log("[HydrationScheduler] All tasks complete")
            }
        ...
        if (useIdleCallback) {
            currentHandle = requestIdleCallback(
                { deadline -> processTasksUntilDeadline(deadline) },
                idleRequestOptions(idleTimeout)
            )
        } else {
            // Fallback to requestAnimationFrame with manual time slicing
            currentHandle = requestAnimationFrame { processTasksWithTimeSlice() }
        }
    }
    ...
    private fun processTasksUntilDeadline(deadline: IdleDeadline) {
        ...
        if (isScrollPaused()) {
            if (enableLogging) {
                console.log("[HydrationScheduler] Paused for scroll, rescheduling")
            }
            scheduleNextBatch()
            return
        }
        ...
        while (taskQueue.isNotEmpty()) {
            val timeRemaining = deadline.timeRemaining()

            // Yield if not enough time for another task
            if (timeRemaining < minTimeForNewTask) {
                if (enableLogging) {
                    console.log("[HydrationScheduler] Yielding (timeRemaining: ${timeRemaining}ms, processed: $tasksProcessed)")
                }
                break
            }
            ...
            if (isScrollPaused()) {
                if (enableLogging) {
                    console.log("[HydrationScheduler] Scroll detected mid-batch, yielding (processed: $tasksProcessed)")
                }
                break
            }
        ...
        if (enableLogging && tasksProcessed > 0) {
            console.log("[HydrationScheduler] Batch complete: $tasksProcessed tasks in ${elapsed.toInt()}ms")
        }
    ...
    private fun processTasksWithTimeSlice() {
        ...
        while (taskQueue.isNotEmpty()) {
            ...
            if (elapsed >= maxBlockingTimeMs) {
                if (enableLogging) {
                    console.log("[HydrationScheduler] Yielding (elapsed: ${elapsed.toInt()}ms, processed: $tasksProcessed)")
                }
                break
            }
    ...
    private fun executeTask(task: HydrationTask) {
        ...
        try {
            ...
            if (enableLogging) {
                console.log("[HydrationScheduler] Task ${task.id} completed in ${taskDuration.toInt()}ms (success: $success)")
            }

            // Warn if individual task takes too long (> 10ms is concerning)
            if (taskDuration > 10.0) {
                console.log("[HydrationScheduler] WARNING: Task ${task.id} took ${taskDuration.toInt()}ms - consider splitting")
            }
            ...
            val taskDuration = performanceNow() - taskStart
            console.error("[HydrationScheduler] Task ${task.id} failed after ${taskDuration.toInt()}ms: ${e.message}")
            task.onError(e)
        }
    }
