...
class ProgressBarTest {
    ...
    fun testProgressBarWithSpecificValue() {
        ...
        assertEquals(
            testProgress,
            mockRenderer.lastProgressValueRendered,
            "Progress value mismatch"
        ) // Updated property
        assertEquals(
            ProgressType.LINEAR,
            mockRenderer.lastProgressTypeRendered,
            "Progress type should be LINEAR"
        ) // Updated property
        ...
        assertEquals(
            "#f0f0f0",
            capturedModifier.styles["background-color"],
            "Default background style not applied/overridden"
        )
        assertEquals(
            "4px",
            capturedModifier.styles["border-radius"],
            "Default border-radius style not applied/overridden"
        )
    }
    ...
    fun testProgressBarIndeterminate() {
        ...
        assertNull(
            mockRenderer.lastProgressValueRendered,
            "Progress value should be null for indeterminate"
        ) // Updated property
        assertEquals(
            ProgressType.LINEAR,
            mockRenderer.lastProgressTypeRendered,
            "Progress type should be LINEAR for indeterminate"
        ) // Updated property
        assertNotNull(
            mockRenderer.lastProgressModifierRendered,
            "Modifier should not be null for indeterminate"
        ) // Updated property
