...
/**
 * Priority-based hydration queue with viewport-aware scheduling (WASM version).
 *
...
class PriorityHydrationQueue private constructor() {
    ...
    fun enqueue(elementId: String, priority: HydrationPriority = HydrationPriority.VISIBLE, work: () -> Boolean) {
        ...
        if (enableLogging) {
            wasmConsoleLog("[PriorityHydrationQueue] Enqueued $elementId with priority $priority")
        }
    }
     ...
     * Elements entering viewport will be promoted to VISIBLE priority.
     */
    fun startObserving() {
        ...
        if (enableLogging) {
            wasmConsoleLog("[PriorityHydrationQueue] Observation started (using scroll-based detection)")
        }

        // Use scroll-based detection for WASM (simpler than IntersectionObserver interop)
    ...
    fun promoteToVisible(elementId: String) {
        ...
        if (enableLogging) {
            wasmConsoleLog("[PriorityHydrationQueue] Promoted $elementId to VISIBLE")
        }
    }
    ...
    fun drainToScheduler(scheduler: HydrationScheduler) {
        ...
        if (enableLogging && count > 0) {
            wasmConsoleLog("[PriorityHydrationQueue] Drained $count tasks to scheduler")
        }
