...
/**
 * DOM operation batcher that implements read-write phase separation (WASM version).
 *
...
class DOMBatcher private constructor() {
     ...
     */
    fun flush() {
        rafHandle?.let { wasmCancelAnimationFrame(it) }
        rafHandle = null
        executeFlush()
    }
    ...
    private fun scheduleFlush() {
        ...
        rafHandle = wasmRequestAnimationFrame { executeFlush() }
    }
    ...
    private fun executeFlush() {
        ...
        if (enableLogging && (readCount > 0 || writeCount > 0)) {
            wasmConsoleLog("[DOMBatcher] Flushing: $readCount reads, $writeCount writes")
        }
        ...
        while (readQueue.isNotEmpty()) {
            ...
            try {
                ...
                wasmConsoleError("[DOMBatcher] Read operation failed: ${e.message}")
            }
        }
        ...
        while (writeQueue.isNotEmpty()) {
            ...
            try {
                ...
                wasmConsoleError("[DOMBatcher] Write operation failed: ${e.message}")
            }
        }
        ...
        if (enableLogging && (readCount > 0 || writeCount > 0)) {
            wasmConsoleLog("[DOMBatcher] Flush complete: ${elapsed.toInt()}ms")
        }
