...
```bash
java -jar summon-cli-0.4.9.3.jar init portal --mode=fullstack --backend=quarkus  # Skip prompts
java -jar summon-cli-0.4.9.3.jar init marketing --mode=standalone --here          # Generate in current dir
```
...
```bash
java -jar summon-cli-0.4.9.3.jar init landing --mode=standalone
cd landing
...
```bash
java -jar summon-cli-0.4.9.3.jar init portal --mode=fullstack --backend=quarkus
cd portal
./gradlew build
./gradlew unitTest
./gradlew quarkusDev
...
```bash
java -jar summon-cli/build/libs/summon-cli-0.4.9.3.jar --help
```
