...
class WebController @Autowired constructor(
    ...
    fun home(
        ...
        return try {
            ...
            renderFullPage(
                renderer = summonRenderer.renderer,
                title = "Error",
                message = "Error loading home page: ${e.message}",
                messageType = "error",
                theme = theme,
                currentPage = "home"
            ) {
                Div(
                    ...
                    Text("An error occurred while loading the home page.")
                }
            }
        }
    }
    ...
    fun users(
        ...
        // Get theme from session, fall back to URL parameter, then default to light
        val sessionTheme = request.session.getAttribute("theme") as? String
        val currentTheme = sessionTheme ?: theme
    ...
    /**
     * Render the chat page with theming support.
     * Returns pure HTML generated by Summon - no templates needed.
     */
    ...
    fun chat(
        ...
        return try {
            ...
            renderFullPage(
                ...
                theme = currentTheme,
                currentPage = "chat",
                includeJavaScript = true, // Include JavaScript for WebSocket functionality
                additionalScripts = listOf("/chat-websocket.js") // Include chat WebSocket client
            ) {
                ChatPage()
            }
        } catch (e: Exception) {
    ...
    fun todos(
        ...
        // Get theme from session, fall back to URL parameter, then default to light
        val sessionTheme = request.session.getAttribute("theme") as? String
        val currentTheme = sessionTheme ?: theme ?: "light"
        
        logger.info("=== WEBCONTROLLER THEME DEBUG ===")
        logger.info("Session ID: ${request.session.id}")
        logger.info("Theme from session: $sessionTheme")
        logger.info("Theme from URL param: $theme")
        logger.info("Final theme to use: $currentTheme")
        
        // Store theme in session for future requests if it was passed as parameter
        if (theme != null) {
            request.session.setAttribute("theme", theme)
            logger.info("Stored theme '$theme' in session")
        }
        
        logger.info("Rendering todos page with theme: $currentTheme")
        ...
        return try {
            ...
            // Render complete HTML page using pure Summon components with ThemeProvider
            renderFullPage(
                renderer = summonRenderer.renderer,
